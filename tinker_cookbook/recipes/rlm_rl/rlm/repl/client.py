...
Runs in a child interpreter (``worker.py``) under ``RLIMIT_AS`` and is killed if it
...
class PythonRepl:
    ...
    def set_variable(self, name: str, value: Any) -> None:
        """Bind a name in the worker namespace. Safe before the first ``execute``
        (and after a reset): the next worker start sends the current ``_variables``."""
    ...
    def _send_sync(self, obj: Any) -> None:
        ...
        _protocol.send(proc.stdin, obj)

    def _recv_sync(self) -> Any:
        proc = self._proc
        if proc is None or proc.stdout is None:
            raise EOFError("REPL worker is not running")
        msg = _protocol.recv(proc.stdout)
        if msg is None:
            raise EOFError("REPL worker closed the pipe")
        return msg
    ...
    def _reset_error(self, note: str, message: str) -> ExecResult:
        """Kill the worker, mark the namespace pending-reset, and return ``message`` prefixed
        with any ``note`` from a prior reset."""
        self._teardown()
        self._reset_pending = True
        return ExecResult("", note + message)
    ...
    async def execute(self, code: str) -> ExecResult:
        ...
        try:
            ...
            result = await self._service_until_done()
        except TimeoutError:
            return self._reset_error(
                note,
                f"REPLTimeout: this block used more than {self._compute_timeout_s:.0f}s of "
            ...
            return self._reset_error(
                note,
                f"REPLReset: the REPL was restarted after an internal failure "
    ...
    async def _service_until_done(self) -> ExecResult:
