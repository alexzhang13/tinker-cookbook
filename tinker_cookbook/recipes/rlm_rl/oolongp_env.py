...
The dataset ships questions and gold pair sets but no context; the context comes from
`oolongbench/oolong-synth` trec_coarse at the matching `context_len`, where every example of a
given length shares one context window. Gold answers were computed from the *labelled* context;
the model is shown the unlabelled one and must infer labels itself.

Gold sets run to hundreds of pairs, so exact match would score ~0 on every rollout and give RL
no gradient at all. Reward is set F1 over the pairs parsed out of the model's answer.
"""
...
class OolongPairsDatasetBuilder(RLDatasetBuilder):
    ...
    # Sub-calls default to the policy renderer; they are tool-style extraction, not reasoning.
    sub_renderer_name: str | None = None
    # Hybrid models (e.g. Qwen3.5) recommend a renderer that opens a `<think>` block, so the policy
    # spends its token budget on a reasoning preamble instead of emitting a ```repl``` block.
